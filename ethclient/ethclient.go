...
// StakeholdersAt returns the stakeholders of the given account.
// The block number can be nil, in which case the stakeholders are taken from the latest known block.
func (ec *Client) StakeholdersAt(ctx context.Context, account common.Address, blockNumber *big.Int) ([]common.Address, error) {
	var result []common.Address
	err := ec.c.CallContext(ctx, &result, "eth_getStakeholders", account, toBlockNumArg(blockNumber))
	return result, err
}

func (ec *Client) ProposalNumberAt(ctx context.Context, account common.Address, blockNumber *big.Int) (uint64, error) {
	var result uint64
	err := ec.c.CallContext(ctx, &result, "eth_getProposalNumber", account, toBlockNumArg(blockNumber))
	return result, err
}

func (ec *Client) VotesNeededToWinAt(ctx context.Context, account common.Address, blockNumber *big.Int) (uint64, error) {
	var result uint64
	err := ec.c.CallContext(ctx, &result, "eth_getVotesNeededToWin", account, toBlockNumArg(blockNumber))
	return result, err
}
