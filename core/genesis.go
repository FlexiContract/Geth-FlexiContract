...
//go:generate go run github.com/fjl/gencodec -type GenesisProposal -out gen_genesis_proposal.go
//go:generate go run github.com/fjl/gencodec -type GenesisVote -out gen_genesis_vote.go
	...
	for addr, account := range *ga {
		...
		for proposalNumber, genesisProposal := range account.Proposals {
			...
			for voterAddress, genesisVote := range genesisProposal.Votes {
				...
				statedb.SetVote(addr, proposalNumber, voterAddress, vote)
			}
		}
	}
...
func CommitGenesisState(db ethdb.Database, triedb *trie.Database, hash common.Hash) error {
	...
	return alloc.flush(db, triedb, hash)
}
...
type genesisAccountMarshaling struct {
	ProposalNumber math.HexOrDecimal64
	Code           hexutil.Bytes
