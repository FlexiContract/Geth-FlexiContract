...
func (db *odrDatabase) OpenProposalTrie(state, addrHash, root common.Hash) (state.Trie, error) {
	return &odrTrie{db: db, id: ProposalTrieID(db.id, addrHash, root)}, nil
}

func (db *odrDatabase) OpenBallotTrie(state, addrHash, root common.Hash) (state.Trie, error) {
	return &odrTrie{db: db, id: ProposalTrieID(db.id, addrHash, root)}, nil
}
