...
// GetStakeholders returns the stakeholders for the given address in the state of the
// given block number.The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta
// block numbers are also allowed.
func (s *BlockChainAPI) GetStakeholders(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) ([]common.Address, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return nil, err
	}
	return state.GetStakeholders(address), state.Error()
}

func (s *BlockChainAPI) GetProposalNumber(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (uint64, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return 0, err
	}
	return state.GetProposalNumber(address), state.Error()
}

func (s *BlockChainAPI) GetVotesNeededToWin(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (uint64, error) {
	state, _, err := s.b.StateAndHeaderByNumberOrHash(ctx, blockNrOrHash)
	if state == nil || err != nil {
		return 0, err
	}
...
func (s *BlockChainAPI) GetExecutionState(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (uint8, error) {
	...
	proposalNumber := state.GetProposalNumber(address)

	if proposalNumber == 0 {
		return types.ChangesApplied, state.Error()
	} else {
		currentState := state.GetProposal(address, proposalNumber).CurrentState
...
func newRPCTransaction(tx *types.Transaction, blockHash common.Hash, blockNumber uint64, index uint64, baseFee *big.Int, config *params.ChainConfig) *RPCTransaction {
	...
	case types.SmartContractUpdateTxType:
		al := tx.AccessList()
		reogList := tx.ReorgList()
