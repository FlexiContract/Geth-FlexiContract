...
const (
	...
	AccessListTxType
	DynamicFeeTxType
	SmartContractUpdateTxType
	ApproveProposalTxType
	RejectProposalTxType
)
...
func (tx *Transaction) decodeTyped(b []byte) (TxData, error) {
	...
	case DynamicFeeTxType:
		...
		err := rlp.DecodeBytes(b[1:], &inner)
		return &inner, err
	case SmartContractUpdateTxType:
		var inner SmartContractUpdateTx
		err := rlp.DecodeBytes(b[1:], &inner)
		return &inner, err
	case ApproveProposalTxType:
		var inner ApproveProposalTx
		err := rlp.DecodeBytes(b[1:], &inner)
		return &inner, err
	case RejectProposalTxType:
		var inner RejectProposalTx
		err := rlp.DecodeBytes(b[1:], &inner)
		return &inner, err
	default:
...
func (tx *Transaction) ReorgList() ReorgList { return tx.inner.reorgList() }

func (tx *Transaction) DataTypes() DataTypes { return tx.inner.dataTypes() }

// Stakeholders returns the account stakeholders
func (tx *Transaction) Stakeholders() []common.Address { return tx.inner.stakeholders() }
...
func UniqueStakeholders(stkhldrs []common.Address) []common.Address {

	seen := make(map[common.Address]bool)

	uniqueStkhldrs := make([]common.Address, 0)

	for _, address := range stkhldrs {
		// If the element is not seen yet, add it to the unique list and mark it as seen
		if _, exists := seen[address]; !exists {
			uniqueStkhldrs = append(uniqueStkhldrs, address)
			seen[address] = true
		}
	}

	return uniqueStkhldrs
}

func UniqueReorgList(reorgInfos []ReorgInfo) []ReorgInfo {

	seen := make(map[ReorgInfo]bool)

	uniqueReorgInfoList := make([]ReorgInfo, 0)

	for _, reorgInfo := range reorgInfos {
		// If the element is not seen yet, add it to the unique list and mark it as seen
		if _, exists := seen[reorgInfo]; !exists {
			uniqueReorgInfoList = append(uniqueReorgInfoList, reorgInfo)
			seen[reorgInfo] = true
		}
	}

	return uniqueReorgInfoList
}
